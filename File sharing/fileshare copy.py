...
# ─── Configuration ────────────────────────────────────────────────────────────
SHARE_DIR   = Path.home() / "Desktop/FileShare"   # Folder to share (created if missing)
PORT        = 8080
# ──────────────────────────────────────────────────────────────────────────────
...
def make_qr(url: str) -> str:
    ...
    qr.add_data(url)
    qr.make(fit=True)
    # Render directly to string for simple terminal output.
    import io
    buf = io.StringIO()
    import sys
...
def build_hotspot_help_html() -> str:
    return """
<div class="card">
  <p style="font-weight:600;margin-bottom:.7rem">Hotspot setup (Phone -> Mac)</p>
  <p class="sub" style="margin-bottom:.6rem">Use this when you want direct local transfer over phone hotspot.</p>
  <ol style="padding-left:1.1rem;line-height:1.7;color:#d7d9e5;font-size:.9rem">
    <li>Turn on hotspot on your phone.</li>
    <li>Connect your Mac to that phone hotspot.</li>
    <li>Start this server on your Mac.</li>
    <li>On phone browser, open the Local URL shown in terminal.</li>
  </ol>
</div>
"""


def print_startup_guide(url: str):
        print("\n" + "═" * 60)
        print("  FileShare — Local Network Transfer (Wi-Fi / Hotspot)")
        print("═" * 60)
        print(f"  Local URL   : {url}")
        print(f"  Share folder: {SHARE_DIR}")
        print("─" * 60)
        print("  HOTSPOT FLOW (Phone shares hotspot):")
        print("  1. Turn ON hotspot on phone")
        print("  2. Connect Mac to phone hotspot")
        print(f"  3. Open this URL on phone browser: {url}")
        print("  4. Upload / Download files")
        print("─" * 60)
        print("  NOTE: Works on same local network only (no internet server).")
        print("─" * 60)
...
class FileShareHandler(BaseHTTPRequestHandler):
    ...
    def _serve_index(self, message="", msg_type=""):
        ...
        hotspot_help = build_hotspot_help_html()

        html = f"""<!DOCTYPE html><html><head><title>FileShare</title>{HTML_STYLE}</head><body>
<h1>📡 FileShare</h1>
    <p class="sub">Mac ↔ Android local transfer over Wi-Fi or phone hotspot</p>
{msg_html}
<div class="card">
